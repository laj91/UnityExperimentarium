...
{
    [Header("Dynamic Destruction")]
    public bool dynamicDestructionEnabled;
    public int health = 100;
    public float destroyForce = 2f;
    ...
    private GameObject CreateFragmentTemplate()
    {
        ...
        if (dynamicDestructionEnabled)
        {
            CopyDynamicComponent(obj);
        }
        return obj;
    }
    ...
    private void CopyDynamicComponent(GameObject obj)
    {
        var dynamicComponent = obj.AddComponent<DynamicDestruction>();

        // Sæt værdier på den nye komponent fra originalen
        dynamicComponent.SetHealth(this.health);
        dynamicComponent.SetForce(this.destroyForce);
        dynamicComponent.SetDelay(this.reenableCollisionDelay);

        // Rigidbody indstillinger
        Rigidbody rigidBody = obj.GetComponent<Rigidbody>();
        rigidBody.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
        rigidBody.isKinematic = true;
        rigidBody.useGravity = false;

        //Collider indstillinger:
